            ...
            <div class="col-12">
                ...
                <div class="bg-light border border-primary rounded p-5">
                    ...
                    <div class="alert alert-info alert-dismissible fade show mb-4" role="alert">
                        ...
                        <a href="{{ route('pindah.index') }}" class="btn btn-sm btn-outline-info ms-2">Hapus Filter</a>
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
                    </div>
                    @endif
                            ...
                            <div class="card h-100 shadow-sm border-primary card-hover">
                                ...
                                <div class="card-header bg-primary text-white p-3 border-0 d-flex align-items-center">
                                    <div class="flex-shrink-0 me-3">
                                        <div class="avatar-box bg-white text-primary fw-bold rounded-circle d-flex align-items-center justify-content-center shadow-sm">
                                            {{ substr($item->warga->nama ?? 'X', 0, 1) }}
                                        </div>
                                    </div>
                                    <div class="flex-grow-1 overflow-hidden">
                                        <h6 class="mb-1 text-white fw-bold text-truncate" title="{{ $item->warga->nama ?? 'Data Terhapus' }}">
                                            {{ $item->warga->nama ?? 'Data Terhapus' }}
                                        </h6>
                                    ...
                                    <div class="mb-3">
                                        <div class="d-flex align-items-center mb-2">
                                            ...
                                            <span class="fw-bold text-dark">
                                                {{ \Carbon\Carbon::parse($item->tgl_pindah)->format('d F Y') }}
                                            </span>
                                        </div>
                                ...
                                <div class="card-footer bg-transparent border-top-0 p-3 pt-0">
                                    <div class="d-grid gap-2">
                                        <a href="{{ route('pindah.show', $item->pindah_id) }}" class="btn btn-info text-white fw-bold w-100">
                                            <i class="fas fa-file-upload me-2"></i> Detail & Bukti
                                        </a>

                                        <div class="d-flex gap-2">
                                            <a href="{{ route('pindah.edit', $item->pindah_id) }}" class="btn btn-warning text-white flex-grow-1">
                                                <i class="fas fa-edit me-2"></i> Edit
                                            </a>
                                            <form action="{{ route('pindah.destroy', $item->pindah_id) }}" method="POST" class="flex-grow-1" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                                                @csrf @method('DELETE')
