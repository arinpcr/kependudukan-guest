...
<div class="container-fluid py-5">
                ...
                <div class="bg-light border border-primary rounded p-5">
                    ...
                    <div class="card mb-4 border-primary">
                        <div class="card-header bg-primary text-white py-2">
                            <h6 class="mb-0">
                                <i class="fas fa-home me-2"></i>Kartu Keluarga: {{ $keluarga->kk_nomor }}
                            </h6>
                        </div>
                        <div class="card-body py-3">
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <p class="mb-1 fw-bold">{{ $keluarga->kepalaKeluarga->nama }}</p>
                                </div>
                                <div class="col-md-6">
                                    <small class="text-muted">Alamat:</small>
                                    <p class="mb-1 fw-bold">{{ $keluarga->alamat }}</p>
                                </div>
                            </div>
                        </div>
                    </div>

                    <a href="{{ route('anggota-keluarga.index', $keluarga->kk_id) }}" class="btn btn-secondary btn-sm mb-4">
                    ...
                    <form action="{{ route('anggota-keluarga.store', $keluarga->kk_id) }}" method="POST">
                        @csrf
    ...
    <div class="form-floating">
        <select class="form-select @error('warga_id') is-invalid @enderror" 
            ...
            @foreach ($warga as $wargaItem) {{-- ✅ UBAH VARIABLE --}}
                <option value="{{ $wargaItem->warga_id }}" {{ old('warga_id') == $wargaItem->warga_id ? 'selected' : '' }}>
                    {{ $wargaItem->nama }} (KTP: {{ $wargaItem->no_ktp }}) - {{ $wargaItem->jenis_kelamin == 'L' ? 'L' : 'P' }}
                </option>
            @endforeach
        </select>
        <label for="warga_id">
            ...
            Hanya menampilkan warga yang belum menjadi anggota di KK manapun.
                                ...
                                <div class="form-floating">
                                    <select class="form-select @error('hubungan') is-invalid @enderror" 
                                        ...
                                        @foreach ($hubunganOptions as $value => $label)
                                            <option value="{{ $value }}" {{ old('hubungan') == $value ? 'selected' : '' }}>
                                                {{ $label }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-12">
                                <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                                    <a href="{{ route('anggota-keluarga.index', $keluarga->kk_id) }}" class="btn btn-secondary me-md-2 px-4">
